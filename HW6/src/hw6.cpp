...
int main (void)
{
	...
	if(ts.runTests()){
		...
		//	ship ship[NUM_OF_SHIPS] = {{'c', 5, "Carrier"},
		//	                                 {'b', 4, "Battleship"},
		//	                                 {'r', 3, "Cruiser"},
		//	                                 {'s', 3, "Submarine"},
		//	                                 {'d', 2, "Destroyer"}};
		...
		switch (option) {
		case 1: in.manuallyPlaceShipsOnGameBoard (playerOneGameBoard, ship);
		break;
		case 2: in.randomlyPlaceShipsOnGameBoard (playerOneGameBoard, ship);
		break;
		}
		...
		//cout << "Resulting board configurations are - ";
		int count = 0;
		char ask = ' ';
		while (TRUE) {
			...
			if (ask == 'y'){
				...
				case PLAYER_ONE:
					...
					do {
						target = in.getTarget ();
						shot = in.checkShot (playerTwoGameBoard, target);
				...
				case PLAYER_TWO:
					...
					if (targetMode) {
						...
						do {
							if (cardinals[NORTH]) {        /* NORTH */
								...
								if (in.checkShot (playerOneGameBoard, targetTemp) != -1 && north >= 0) {
								...
								if (in.checkShot (playerOneGameBoard, targetTemp) != -1 && south <= 9) {
								...
								if (in.checkShot (playerOneGameBoard, targetTemp) != -1 && west >= 0) {
									cardinals[WEST] = TRUE;
								}
								...
								if (in.checkShot (playerOneGameBoard, targetTemp) != -1 && east <= 9) {
									cardinals[EAST] = TRUE;
								}
							...
							shot = in.checkShot (playerOneGameBoard, target);
					...
					if (huntMode) {
						...
						do {
							target.row = in.getRandomNumber (0, 9);
							target.column = in.getRandomNumber (0, 9);
					...
					if (shot == 1) {
						...
						if (cardinals[NORTH] == TRUE) {  /* NORTH */
							...
							in.checkBoundsOfCardinal (cardinals, north, NORTH);
							targetTemp = target;
							targetTemp.row = north;
							if (in.checkShot (playerOneGameBoard, targetTemp) == -1)
								cardinals[NORTH] = FALSE;
						}

						if (cardinals[SOUTH] == TRUE) {  /* SOUTH */
							south = targetAI.row + 1;
							in.checkBoundsOfCardinal (cardinals, south, SOUTH);
							targetTemp = target;
							targetTemp.row = south;
							if (in.checkShot (playerOneGameBoard, targetTemp) == -1)
						...
						if (cardinals[WEST] == TRUE) {   /* WEST */
							...
							in.checkBoundsOfCardinal (cardinals, west, WEST);
							targetTemp = target;
							targetTemp.column = west;
							if (in.checkShot (playerOneGameBoard, targetTemp) == -1)
						...
						if (cardinals[EAST] == TRUE) {   /* EAST */
							...
							in.checkBoundsOfCardinal (cardinals, east, EAST);
							targetTemp = target;
							targetTemp.column = east;
							if (in.checkShot (playerOneGameBoard, targetTemp) == -1)
				...
				if (shot == 1) { /* HIT */
					...
					if (player == 1)
						hasAShipSunked = in.checkSunkShip (sunkShip, !player, shipSymbol, outStream);
					else
						in.checkSunkShip (sunkShip, !player, shipSymbol, outStream);
